...
<form method="post" name="adminForm" id="adminForm" action="<?php echo Route::_('index.php?option=com_eventbooking&view=location&location_id=' . $this->location->id . '&Itemid=' . $this->Itemid) ; ?>">
	...
	if (count($this->items))
	{
		...
		{
			echo EventbookingHelperHtml::loadCommonLayout('common/events_timeline.php', ['events' => $this->items, 'config' => $this->config, 'Itemid' => $this->Itemid, 'nullDate' => $this->nullDate, 'ssl' => (int) $this->config->use_https, 'viewLevels' => $this->viewLevels, 'category' => null, 'Itemid' => $this->Itemid, 'bootstrapHelper' => $this->bootstrapHelper]);
		}
		else
		{
			$this->categoryId =  0;
